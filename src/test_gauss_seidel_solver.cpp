...
public:
    ...
    TestCaseGaussSeidelSolver( const string& case_name, const int dim, const int iteration )
...
class TestCaseGaussSeidelSolver_baseline : public TestCaseGaussSeidelSolver< T, IS_COL_MAJOR > {
  ...
  public:
    ...
    TestCaseGaussSeidelSolver_baseline( const string& case_name, const int dim, const int solver_iteration )
        :TestCaseGaussSeidelSolver< T, IS_COL_MAJOR >{ case_name, dim, solver_iteration }
        ,m_Dinv                                      { new T[dim] }
    {
        ;
    }
...
class TestCaseGaussSeidelSolver_NEON : public TestCaseGaussSeidelSolver< T, IS_COL_MAJOR > {
  ...
  public:
    ...
    TestCaseGaussSeidelSolver_NEON( const string& case_name, const int dim, const int solver_iteration, const int factor_loop_unrolling )
        :TestCaseGaussSeidelSolver< T, IS_COL_MAJOR >{ case_name, dim, solver_iteration }
        ,m_Dinv                                      { new T[dim] }
...
string testSuitePerType( const bool print_diag, const T condition_num, const T gen_low, const T gen_high )
{
    ...
    if constexpr ( !IS_COL_MAJOR ) {
        ...
        case_names.push_back( "NEON loop unrolled order 1" );
        case_names.push_back( "NEON loop unrolled order 2" );
        case_names.push_back( "NEON loop unrolled order 4" );
        case_names.push_back( "NEON loop unrolled order 8" );
    }
    ...
    for( auto& dim : matrix_dims ) {
        ...
        TestExecutorGaussSeidelSolver<T, IS_COL_MAJOR> e( results, dim, condition_num, gen_low, gen_high, NUM_TRIALS, false );

        e.addTestCase( make_shared< TestCaseGaussSeidelSolver_baseline    <T, IS_COL_MAJOR> > ( case_names[0], dim, SOLVER_ITERATION ) );

        if constexpr ( !IS_COL_MAJOR ) {

            if ( dim >= neon_num_lanes ) {
                e.addTestCase( make_shared< TestCaseGaussSeidelSolver_NEON  <T, IS_COL_MAJOR> > ( case_names[1], dim, SOLVER_ITERATION, 1 ) );
            }
            if ( dim >= 2 * neon_num_lanes ) {
                e.addTestCase( make_shared< TestCaseGaussSeidelSolver_NEON  <T, IS_COL_MAJOR> > ( case_names[2], dim, SOLVER_ITERATION, 2 ) );
            }
            if ( dim >= 4 * neon_num_lanes ) {
                e.addTestCase( make_shared< TestCaseGaussSeidelSolver_NEON  <T, IS_COL_MAJOR> > ( case_names[3], dim, SOLVER_ITERATION, 4 ) );
            }
            if ( dim >= 8 * neon_num_lanes ) {
                e.addTestCase( make_shared< TestCaseGaussSeidelSolver_NEON  <T, IS_COL_MAJOR> > ( case_names[4], dim, SOLVER_ITERATION, 8 ) );
            }
        }
...
EMSCRIPTEN_BINDINGS( saxpy_module ) {
    emscripten::function( "testGaussSeidelSolverFloatColMajor",  &testGaussSeidelSolverFloatColMajor  );
    emscripten::function( "testGaussSeidelSolverFloatRowMajor",  &testGaussSeidelSolverFloatRowMajor  );
    emscripten::function( "testGaussSeidelSolverDoubleColMajor", &testGaussSeidelSolverDoubleColMajor );
    emscripten::function( "testGaussSeidelSolverDoubleRowMajor", &testGaussSeidelSolverDoubleRowMajor );
}
