#ifndef __TEST_CASE_LCP_LEMKE_NEON_H__
#define __TEST_CASE_LCP_LEMKE_NEON_H__
#include <iostream>
#include <vector>
#include <limits>
#include <string>
...
class TestCaseLCP_lemke_neon : public TestCaseLCP<T, IS_COL_MAJOR> {
  ...
  public:
    ...
    TestCaseLCP_lemke_neon(
        ...
        :TestCaseLCP<T, IS_COL_MAJOR>{ case_name, dim, condition_num, epsilon, p_type }
    ...
    virtual ~TestCaseLCP_lemke_neon()
    {
        delete[] m_table;
    ...
    void  pivot( const int entering_col_index, const int leaving_row_index )
    {
        ...
        for ( size_t row_index = 0; row_index < this->m_dim; row_index++ ) {
            ...
            if ( row_index != leaving_row_index ) {
                ...
                if constexpr( is_same<float, T>::value ) {
                    ...
                    const float64x2_t qw_coeff  = { coeff,  coeff };

                    for ( col_index = 0; col_index <= this->m_num_columns - 4; col_index += 4 ) {

                        const float64x2_t qw_pivot_row1   = vld1q_f64( &(pivot_row  [ col_index ]) );
                        const float64x2_t qw_current_row1 = vld1q_f64( &(current_row[ col_index ]) );
                        const float64x2_t qw_pivot_row2   = vld1q_f64( &(pivot_row  [ col_index + 2]) );
                        const float64x2_t qw_current_row2 = vld1q_f64( &(current_row[ col_index + 2]) );
                        const float64x2_t qw_cal1         = vmulq_f64( qw_pivot_row1, qw_coeff );
                        const float64x2_t qw_cal2         = vmulq_f64( qw_pivot_row2, qw_coeff );
                        const float64x2_t qw_sum1         = vaddq_f64( qw_current_row1, qw_cal1 ); 
                        const float64x2_t qw_sum2         = vaddq_f64( qw_current_row2, qw_cal2 ); 
                        memcpy( &(current_row[ col_index     ]), &qw_sum1, sizeof(T)*2 );
                        memcpy( &(current_row[ col_index + 2 ]), &qw_sum2, sizeof(T)*2 );
                    }
                }
        ...
        if constexpr( is_same<float, T>::value ) {
            const float32x4_t qw_pivot_denom = { pivot_denom, pivot_denom, pivot_denom, pivot_denom };
            for ( col_index = 0; col_index <= this->m_num_columns - 8; col_index += 8 ) {

                const float32x4_t qw_pivot_row1   = vld1q_f32( &(pivot_row[ col_index    ]) );
                const float32x4_t qw_pivot_row2   = vld1q_f32( &(pivot_row[ col_index + 4]) );
                const float32x4_t qw_cal1         = vmulq_f32( qw_pivot_row1, qw_pivot_denom );
                const float32x4_t qw_cal2         = vmulq_f32( qw_pivot_row2, qw_pivot_denom );
                memcpy( &(pivot_row[ col_index     ]), &qw_cal1, sizeof(T)*4 );
                memcpy( &(pivot_row[ col_index + 4 ]), &qw_cal2, sizeof(T)*4 );
            }
        }
        else {
            const float64x2_t qw_pivot_denom = { pivot_denom, pivot_denom };
            for ( col_index = 0; col_index <= this->m_num_columns - 4; col_index += 4 ) {

                const float64x2_t qw_pivot_row1   = vld1q_f64( &(pivot_row[ col_index    ]) );
                const float64x2_t qw_pivot_row2   = vld1q_f64( &(pivot_row[ col_index + 2]) );
                const float64x2_t qw_cal1         = vmulq_f64( qw_pivot_row1, qw_pivot_denom );
                const float64x2_t qw_cal2         = vmulq_f64( qw_pivot_row2, qw_pivot_denom );
                memcpy( &(pivot_row[ col_index     ]), &qw_cal1, sizeof(T)*2 );
                memcpy( &(pivot_row[ col_index + 2 ]), &qw_cal2, sizeof(T)*2 );
            }
        }
